@section('settings_user')
	<div class="row span7">
		...
		<h4>Edit Profile User // <span class="muted">{{ $user->fullname }}</span></h4>
		<div class="bordered">
			...
			{{ Form::hidden('id', $user->id) }}
				<table>
					<tr>
						...
						<td>{{ Form::text('username', $user->username, array('class'=>'span4')) }}</td>
					</tr>
					<tr>
						<td class="field">{{ Form::label('fullname', 'Full Name:')}}</td>
						<td>{{ Form::text('fullname', $user->fullname, array('class'=>'span4')) }}</td>
					</tr>
					<tr>
						<td class="field">{{ Form::label('tipe', 'Tipe:')}}</td>
						<td>{{ Form::select('tipe', $reversed_daftar_tipe, $user->tipe) }}</td>
					</tr>
					<tr>

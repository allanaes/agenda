@section('suratmasuk_pengawasan_selesai_table')
	<div class="row">	
		<h2>Daftar Pengawasan Surat Selesai Diproses</h2>

		@if($pengawasan_surat_selesai->links() != '')
			<div class="pull-left">{{ $pengawasan_surat_selesai->links() }}</div>
		@endif
		...
		<table class="displaytable">
			...
			<?php
				$j = 0;
			?>
			@foreach ($pengawasan_surat as $row)
				@if($j % 2 == 0)
				<tr class="tr-alt">
				@else
				<tr>
				@endif
					<?php $j++ ?>
					<?php $baris_ke = $j + (($pengawasan_surat_selesai->page - 1) * Konfigurasi::find(7)->config_value); ?>
					<td class="align-center">{{ $baris_ke }}</td>
					<td class="span5">
